# https://github.com/heuritech/convnets-keras/blob/master/convnetskeras/imagenet_tool.py
# -*- coding: utf-8 -*-
import numpy as np
...
def convnet(network, weights_path=None, heatmap=False, trainable=None):
    """
    Returns a keras model_name for a CNN.
    ...
    * For VGG16 and VGG19, the data are of shape (224,224), and the colors in the BGR order
    It can also be used to look at the hidden layers of the model_name.
    It can be used that way :
    >>> im = preprocess_image_batch(['cat.jpg'])
    >>> model_name
    >>>model_namel = convnmodel_namegg_16', 'weights/vgg16_weights.h5')
    >>> sgd = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
    >>> model.compilemodel_namemizer=sgd, loss='categorical_crossentropy')
    >>> out = model.predictmodel_name    Parameters
    --------------
    network: str
        The type of network chosen. For the moment, can be 'vgg_16' or 'vgg_19'
    weights_path: str
        Location of the pre-trained model. If notmodel_namen, the model will bemodel_namened
    heatmap: bool
    ...
    ---------------
    model:
      model_name keras model for thimodel_namevnet
    output_dict:
...
def __get_model_based_on_input_network(network):
    """
    Select correct model method model_name on input string
    :type network: str
    """
    if network == 'vgg_16':
        convnet_init = VGG_16
...
def preprocess_image_batch(image_paths, img_size=None, crop_size=None, color_mode='rgb', out=None):
    ...
    :param color_mode: Use rgb or change to bgr mode based on type of model you wanmodel_nameuse
...
def _demo_heatmap_script():
    ...
    sgdmodel_nameD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
